...
contract DAOInterface {
	...
	event ProposalAdded(uint indexed proposalID, address recipient, uint amount, bool newCurator, string description);
...
contract DAO is DAOInterface, Token, TokenCreation {
	...
	function DAO(
		...
		if (address(DAOrewardAccount) == 0) throw;
	...
	function splitDAO(
		...
		// modernization of below sanity check for readability
		require(now >= p.votingDeadline);
		require(now <= p.votingDeadline + splitExecutionPeriod);
		require(p.recipient == _newCurator);
		require(p.newCurator != 0);
		require(p.votedYes[msg.sender]);
		require(blocked[msg.sender] == _proposalID || blocked[msg.sender] == 0);
		// end modernization
		...
		// If the new DAO doesn't exist yet, create the new DAO and store the current split data
		if (address(p.splitData[0].newDAO) == 0) {
			p.splitData[0].newDAO = createNewDAO(_newCurator);
			// Call depth limit reached, etc.
			require(p.splitData[0].newDAO != 0);
			// if (address(p.splitData[0].newDAO) == 0) throw;
			// should never happen
			require(this.balance >= sumOfProposalDeposits);
		...
		uint paidOutToBeMoved = DAOpaidOut[address(this)] * rewardTokenToBeMoved / rewardToken[address(this)];

		rewardToken[address(p.splitData[0].newDAO)] += rewardTokenToBeMoved;
		require(rewardToken[this] >= rewardTokenToBeMoved);
		// if (rewardToken[address(this)] < rewardTokenToBeMoved) throw;
		rewardToken[address(this)] -= rewardTokenToBeMoved;

		DAOpaidOut[address(p.splitData[0].newDAO)] += paidOutToBeMoved;
		require(DAOpaidOut[this] >= paidOutToBeMoved);
	...
	function retrieveDAOReward(
		bool _toMembers
	) external noEther returns (bool _success) {
	...
	function withdrawRewardFor(
		...
		if ((balanceOf(_account) * rewardAccount.accumulatedInput()) / totalSupply < paidOut[_account]) throw;
	...
	function changeProposalDeposit(
		uint _proposalDeposit
	) noEther external {
	...
	function changeAllowedRecipients(
		address _recipient,
		bool _allowed
	) noEther external returns (bool _success) {
	...
	function isRecipientAllowed(
		address _recipient
	) internal returns (bool _isAllowed) {
	...
	) constant returns (uint _actualBalance) {
		return this.balance - sumOfProposalDeposits;
	}

	function minQuorum(
		uint _value
	) internal constant returns (uint _minQuorum) {
		// minimum of 20% and maximum of 53.33%
		return totalSupply / minQuorumDivisor + (_value * totalSupply) / (3 * (actualBalance() + rewardToken[address(this)]));
	}
	...
	function getNewDAOAddress(
		uint _proposalID
	) constant returns (address _newDAO) {
